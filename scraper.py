...
class Scraper:
    def __init__(self, profile_name, options, db, timestamp, debug=False):
    ...
    async def crawl_worker(self, url: str, session: aiohttp.ClientSession):
        ...
        try:
            ...
            self.url_table.append([url, self.timestamp, hash.hexdigest()])
    ...
    async def crawl(self):
        ...
        print(
            f"{self.profile} Crawled: {len(self.visited_urls)} URLs in {time.time() - t_start:.2f} seconds"
        )
        print(f"{self.profile} queue size: {len(self.queue)}")

    async def fetch_page(self, session, root_url):
